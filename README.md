Códigos usados de html y css para la creación de la pagina web.
Pagina web compatible con use de telefonos y pcs o laptops



Historial commits:
Primer commit= Inicio desarrollo evlauación 1 Front-End, se desarrolla a lo largo de las 2 horas la infraestructura basica del navegador y header, proximo commit agregara la imagen de usuario y mejorara el estilo.css
Segundo commit= Finalizacion del navegador para el portafolio evaluacion 1 Front-End se introducen mejoras al css, ademas del logo para la pagina como en portafolio y la imagen de un animal que me gusta, la imagen del estudiante (yo) se vera en la parte de "Sobre Mí"
Tercer commit = añadido de sección "Sobre Mí", trabaje en ambos html y css ademas de agregar una imagen de mi persona, visualizo errores posibles debido a la cantidad de codigo de css que se cruza, arreglar más tarde.
Cuarto commit= inicio y finalizacion de sección "Habilidades", tebaje en ambos html y css, teniendo en cuenta la tabla y las dos listas, quedan errores que arreglar y cambios para la calidad del codigo, arreglar más tarde.